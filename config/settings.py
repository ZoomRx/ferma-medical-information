...
class EnvSettings(JSONSettings):
    ...
    open_ai_key: str = Field(..., alias='OPEN_API_KEY')
...
class DBSettings(JSONSettings):
    ...
    doc_db_name: str = Field(..., alias='DOCUMENT_DB')
    ...
    @property
    def doc_database_url(self) -> str:
        return f"mysql+pymysql://{self.username}:{self.password}@{self.host}/{self.doc_db_name}"
